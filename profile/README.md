...
- ### 진행 인원 및 역할 <br>
  ...
  - 7명 (프론트엔드 4명, 백엔드 3명)<br>
    ...
    | 이가린 | • 마이페이지 프로필 이미지 업로드 부분 | • User API (로그인, 회원가입, 이메일 인증, 개인정보 CRUD) | • 발표<br>• 2차 배포 (AWS, Github Actions) |
    | 이승철 | • 메인페이지<br>• 로그인 및 회원가입 페이지 | - | • PPT 제작<br>• 1차 배포(Nginx) |
...
| • 장바구니 및 최근 검색어 저장 기능 구현             | • multer를 활용한 이미지 처리               |
